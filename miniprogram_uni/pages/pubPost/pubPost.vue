...
<view class="containor">
  <view class="publish_text_area">
    ...
    <view class="text_area_title">
      <input class="title_input" placeholder="请输入标题" maxlength="25" placeholder-style="color:#b3b3b3;font-size:16px;" @input="handleTitleInput" :value="title"></input>
      <!-- 标题字数限制 -->  
    ...
    <view class="text_area_content">
      <view class="area_content">
        <view class="area_content_out">
          <textarea class="content-textarea" placeholder="请输入正文内容..." maxlength="400" placeholder-style="color:#b3b3b3;font-size:12px;" style="height: 8rem" @input="handleContentInput" :value="content"></textarea>
          <!-- 字数限制 -->  
  ...
  <!-- tag selection -->
  <view class="tag_area">
    <view :class="'sign-setting-border ' + (item.isSelect?'selected':'')" v-for="(item, index) in applyList" :key="index" @tap="selectApply" :data-index="index">
      <text decode="true">#{{item.Item_Name}}</text>
    </view>
  </view>
